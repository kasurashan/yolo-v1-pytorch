...
def evaluate(preds,targets,class_names,threshold=0.5):
    ...
    mapList = open('results/mapList.txt', 'a')
    mapList.write(f"{str(np.mean(aps))}\n")
    mapList.close()
...
if __name__ == '__main__':
    ...
    for i in range(135):
        if (i%3)==0:
            epoch_list = epoch_list + [i]

    

    #model_path = 'results/yolo/Oct15_03-39-15/model_best.pth'

    for i in epoch_list:
        model_path = 'results/yolo/Oct15_03-39-15/model_' + str(i) + '.pth'
